...
// Context injection: before_prompt_build pulls `stateroot hook session_start`
// stdout (digest) into prependContext when the API accepts it.
...
async function fireAsync(event: string): Promise<{ stdout: string }> {
  try {
    return await execFileAsync("stateroot", ["hook", event, "--harness", "openclaw"], {
      timeout: 8000,
...
const plugin = {
  ...
  register(api: {
    ...
    api.on("before_prompt_build", async () => {
      const { stdout } = await fireAsync("session_start");
