...
- Before stopping or when nearing limits: write strict structured JSON, then run `stateroot handoff write --from {{CURRENT_HARNESS}} [--to <next-harness>] --input <handoff.json>`. Omit `--to` for continuity-only; use it only when orchestrating a cross-harness switch. Include the durable objective, immediate task, a detailed continuity narrative in `context_summary` (present state, verified evidence, decisions and rationale, constraints, failed approaches, implications for the next agent), structured decisions, truthful failures, and next actions; recent verified conversation is auto-captured. Do not paste giant dumps into legacy `--note`. Normal Windows paths work; `--input -` is only an optional stdin convenience.
