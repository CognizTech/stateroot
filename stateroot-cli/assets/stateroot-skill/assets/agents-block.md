...
5. **Learnings (global + project)** — two layers. Global (`stateroot learn record --user "…"`) is taste that follows the user across projects. Project (`stateroot learn record "…"`) is this-repo convention. After `stateroot init`, the first harness session must seed whichever layer is empty. Records take effect immediately so the next harness inherits them. Every later harness reads both (`stateroot learnings list` and `stateroot learnings list --user`) and updates them when the user corrects you or a durable preference appears.
