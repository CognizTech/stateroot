...
3. **Session end / usage limit / harness switch** — write strict structured JSON and run `stateroot handoff write --from <resolved-current-harness> --to <harness> --input <handoff.json>`. Include durable objective, immediate task, a detailed continuity narrative in `context_summary`, decisions with rationale, truthful failures, and next actions; verified recent conversation is auto-captured. Resolve `--from` explicitly, use normal Windows paths as needed, and do not paste giant dumps into legacy `--note` (`--input -` is only an optional stdin convenience).
