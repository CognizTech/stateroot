...
4b. **Planning for another harness to implement** — record the plan in the shared store, never only in your harness's native plan location (`~/.claude/plans/`, `~/.cursor/plans/`, plan-mode files): `stateroot plan record --stdin --title "…"` (or `--file`). Then hand off with `--to <harness>` — the executor's digest says *execute it; do not re-plan*. Native plan files still federate in as drafts at session boundaries; record-then-handoff is the deliberate path when a plan is meant for another harness now.
