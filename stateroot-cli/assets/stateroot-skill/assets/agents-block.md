...
3. **Before attempting an approach** — run `stateroot search "failed approach <topic>"` (or read `failed_approaches` in the current handoff) and do not repeat recorded failures.
